...
export class LeadsService extends TypeOrmCrudService<Lead> {
  ...
  async getLeadsResults(
    ...
    query = query.groupBy('dealer').addGroupBy('filterYearMonth');
  ...
  async getTotalValidLeads(
    ...
    return await this.repo
      ...
      .innerJoin('dd.dealerCity', 'dc')
      .innerJoin('dc.dealerGroup', 'dg')
      // Status must be SIEBEL
      .where('l.status = :status', { status: LeadStatus.SIEBEL })
      // Lead has to be valid
      .andWhere('l.isValid = true')
      // Lead has to be from the same dealer city
      .andWhere('dc.id = :dealerCityId', { dealerCityId })
      // Dealer city has not not to be ignored
      .andWhere('dc.ignore = false')
      // Lead has to be created in the same month
      .andWhere("DATE_FORMAT(l.creationDate, '%Y') = :year", {
        year,
      })
      .andWhere("DATE_FORMAT(l.creationDate, '%m') like '%:month%'", {
        month,
      })
      .getCount();
  }

  async getTotalLeadsThatHasASale(
    dealerCityId: number,
    leadMonth: number,
    leadYear: number,
    saleMonth: number,
    ...
    return await this.repo
      ...
      .innerJoin('dd.dealerCity', 'dc')
      .innerJoin('dc.dealerGroup', 'dg')
      // THIS JOIN TELLS THE QUERY IF THE SALE IS AN OVERALL OR FRANCHISE ONE
      .innerJoin('l.sales', 's')
      // Status must be SIEBEL
      .where('l.status = :status', { status: LeadStatus.SIEBEL })
      // Lead has to be valid
      ...
      .andWhere('dc.ignore = false')
      // Lead has to be created in the same month
      .andWhere("DATE_FORMAT(l.creationDate, '%Y') = :leadYear", {
        leadYear,
      })
      .andWhere("DATE_FORMAT(l.creationDate, '%m') like '%:leadMonth%'", {
        leadMonth,
      })
      // Sale has to be created in the same month
      .andWhere('s.month = :saleMonth', { saleMonth })
      .andWhere('s.year = :saleYear', { saleYear })
      .getCount();
  }
}

...
import { T1FunnelDto } from './dto/T1Funnel.dto';
...
export class T1FunnelsController {
  constructor(
    private usersService: UsersService,
    private t1Service: T1FunnelsService,
  ...
  async searchGlobalFunnels(@Request() req) {
    ...
    for (const period of periods) {
      ...
      let funnelsForMonth: T1Funnel[];
      if (dealerToLookFor.id === 'all') {
        funnelsForMonth = await this.t1Service.findByYearMonth(
          periodParts[0],
          parseInt(periodParts[1]),
        );
      } else {
        funnelsForMonth = await this.t1Service.findAllByYearMonthAndDealer(
          periodParts[0],
          parseInt(periodParts[1]),
          dealerToLookFor.id,
        );
      ...
      if (funnelsForMonth.length > 0) {
        response.push({
          ...this.t1Service.mergeT1Funnels(funnelsForMonth),
        ...
        response.push({
          ...
          totalLeads: totalLeads.count,
          totalSales: totalSales.count,
          ...this.t1Service.getEmptyT1Funnel(),
        });
      }
    }
  ...
  @Roles('dealer', 'admin', 'gm')
  @Get(':dealerId')
  async searchT1Funnel(@Param('dealerId') dealerId: number) {
    ...
    const sales = await this.salesService.getYearSalesForDealer(dealerId);
    ...
    for (const period of periods) {
      ...
      const totalSales = sales.find(s => s.period === period);
      const periodParts = period.split('-');
      const funnelForMonth = await this.t1Service.findByYearMonthAndDealer(
        periodParts[0],
        parseInt(periodParts[1]),
        dealerId,
      );
  ...
  async createT1Funnel(
    @Param('dealerId') dealerId: number,
    @Param('year') year: number,
    @Param('month') month: number,
    @Body() body: T1FunnelDto,
  ) {
    ...
    const funnel = await this.t1Service.findByYearMonthAndDealer(
      year,
      searchMonth,
      dealerId,
    );
    if (funnel) {
      await this.t1Service.repo.update(funnel.id, body);
      const updatedFunnel = await this.t1Service.findOne(funnel.id, {
        relations: ['dealer'],
      });
      await this.dealerGroupService.increaseSavesCounter(dealerId);
      return updatedFunnel;
    } else {
      const newFunnel = this.t1Service.repo.create({
        month: searchMonth,
        year,
        dealer: { id: dealerId },
        ...body,
      });
      const { id } = await this.t1Service.repo.save(newFunnel);
      const createdFunnel = await this.t1Service.findOne(id, {
        relations: ['dealer'],
      });

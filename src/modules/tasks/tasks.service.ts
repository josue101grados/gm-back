...
export class TasksService extends TypeOrmCrudService<Task> {
  ...
  async executeTask(task: Task) {
    ...
    switch (task.type) {
      ...
      case TaskTypes.REPROCESS_SALES_MONTH: {
        const { data: salesData } = task;
        if (salesData.year && salesData.month) {
          await this.salesService.reprocessSalesWithLeads(
            Number(salesData.year),
            Number(salesData.month),
          );
        }
      ...
      case TaskTypes.REPROCESS_SALES_YEAR: {
        const { data: salesData } = task;
        if (salesData.year) {
          await this.salesService.reprocessSalesWithLeadsOfWholeYear(
            Number(salesData.year),
          );
        }
        break;
      }
      case TaskTypes.ASSSIGN_SALES_MONTH: {
        const { data: salesData } = task;
        if (salesData.year && salesData.month) {
          await this.salesService.assignSales(
            Number(salesData.year),
            Number(salesData.month),
          );
        }
      ...
      case TaskTypes.ASSSIGN_SALES_YEAR: {
        const { data: salesData } = task;
        if (salesData.year) {
          await this.salesService.assignSalesOfWholeYear(
            Number(salesData.year),
          );
        }
        break;
